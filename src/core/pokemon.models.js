...
export const pokemonSpecies = (resData) => {
	let description = resData.flavor_text_entries.map(
		(flavor) => flavor.language.name === 'en' && flavor.flavor_text
	)
	...
	const eggGroups = resData.egg_groups
		.map((group) =>
			group.name
				.toLowerCase()
				.split(' ')
